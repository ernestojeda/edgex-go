...
func TestValidate(t *testing.T) {
	...
	variations := []struct {
		...
		{
			name:             "valid",
			request:          dto.NewRequest(dtoBase.NewRequest(requestID)),
			expectedResponse: dto.NewRequest(dtoBase.NewRequest(requestID)),
		...
		},
		{
			...
			expectedResponse: dtoError.NewResponse(
				dtoBase.NewResponse(
					"",
					string(test.Marshal(t, dto.NewRequest(dtoBase.NewRequest("")))),
		...
		t.Run(
			...
			func(t *testing.T) {
				result, status := Validate(
					...
					application.NewBehavior(application.Version2, useCase.Kind, application.ActionCommand),
