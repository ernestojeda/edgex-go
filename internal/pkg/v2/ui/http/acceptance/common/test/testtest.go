		...
		for _, variation := range requestVariations {
			for _, m := range test.ValidMethods() {
				t.Run(
					...
					func(t *testing.T) {
						timer := test.NewTimer()
						w, correlationID := test.SendRequest(t, router, m, controller.Endpoint, variation.request)
						timer.Stop()
	...
	for delayVariationIndex := range delayVariations {
		...
		for _, variation := range requestVariations {
			t.Run(
				...
				func(t *testing.T) {
					timer := test.NewTimer()
					w, correlationID := test.SendRequest(t, router, batch.Method, batch.Endpoint, variation.request)
					timer.Stop()
