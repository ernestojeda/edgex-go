			...
			expectedResponse: []interface{}{
				batchdto.NewTestRequest(
					...
					dtoErrorV2dot0.NewResponse(
						dtoBaseV2dot0.NewResponse(
							"",
							string(test.Marshal(t, string(invalidJSON))),
							application.StatusBatchUnmarshalFailure,
						),
					),
				),
			...
			expectedResponse: []interface{}{
				batchdto.NewTestRequest(
					...
					dtoErrorV2dot0.NewResponse(
						dtoBaseV2dot0.NewResponse(
							"",
							string(test.Marshal(t, dtoBaseV2dot0.NewRequest(""))),
	...
	for _, variation := range testVariations {
		for _, m := range test.ValidMethods() {
			t.Run(
				...
				func(t *testing.T) {
					w, correlationID := test.SendRequest(t, router, m, batch.Endpoint, variation.request)
